...
# This script is for Linux/macOS users, if you are on Windows, please use the corresponding shell script in build-CBTC.ps1.
# This script was copied from my build_interim.sh with necessary modifications from my last project at https://github.com/Nanda128/Final-Year-Project/blob/main/src/scripts/build_interim.sh
...
BIB_SOURCE="$PROJECT_ROOT/main.bib"
BIB_DEST="$PARENT_DIR/main.bib"
if [ -f "$BIB_SOURCE" ]; then
  cp -f "$BIB_SOURCE" "$BIB_DEST"
fi

OUTPUT_PATH="$PARENT_DIR/CBTC-CloudWorx_23070854.pdf"

pdflatex -interaction=nonstopmode -halt-on-error -output-directory="$PARENT_DIR" main.tex 2>&1 | tee "$LOG_DIR/pdflatex-pass1.scripts.log"
...
pdflatex -interaction=nonstopmode -halt-on-error -output-directory="$PARENT_DIR" main.tex 2>&1 | tee "$LOG_DIR/pdflatex-pass2.scripts.log"
pdflatex -interaction=nonstopmode -halt-on-error -output-directory="$PARENT_DIR" main.tex 2>&1 | tee "$LOG_DIR/pdflatex-pass3.scripts.log"

TEMP_PDF_PATH="$PARENT_DIR/main.pdf"
if [ -f "$TEMP_PDF_PATH" ]; then
  mv "$TEMP_PDF_PATH" "$OUTPUT_PATH"

  if [ -f "$PARENT_DIR/main.aux" ]; then
    mv "$PARENT_DIR/main.aux" "$AUXIL_DIR/"
  fi
  if [ -f "$PARENT_DIR/main.log" ]; then
    mv "$PARENT_DIR/main.log" "$AUXIL_DIR/"
  fi
  if [ -f "$PARENT_DIR/main.bbl" ]; then
    mv "$PARENT_DIR/main.bbl" "$AUXIL_DIR/"
  fi
  if [ -f "$PARENT_DIR/main.blg" ]; then
    mv "$PARENT_DIR/main.blg" "$AUXIL_DIR/"
  fi
