...
# This script is for Windows users, if you are on Linux or macOS, please use the corresponding shell script in build-CBTC.sh.
# This script was copied from my build_interim.ps1 with necessary modifications from my last project at https://github.com/Nanda128/Final-Year-Project/blob/main/src/scripts/build_interim.ps1
...
$outputPath = Join-Path $repoRoot "CBTC-CloudWorx_23070854.pdf"

$texPath = Join-Path $srcDir 'main.tex'
$bibPathSrc = Join-Path $srcDir 'main.bib'
...
if (Test-Path (Join-Path $repoRoot 'main.aux'))
{
    if (Test-Path $bibPathSrc)
    {
        Copy-Item $bibPathSrc (Join-Path $repoRoot 'main.bib') -Force
    }

    Push-Location $repoRoot
    try
    {
        bibtex main 2>&1 | Tee-Object -FilePath (Join-Path $logDir 'bibtex.scripts.log')
    }
    finally
    {
        Pop-Location
    ...
    if (Test-Path (Join-Path $repoRoot 'main.bib'))
    {
        Remove-Item (Join-Path $repoRoot 'main.bib') -Force
    }
}
...
$tempPdfPath = Join-Path $repoRoot "main.pdf"
if (Test-Path $tempPdfPath)
{
    Move-Item $tempPdfPath $outputPath -Force

    $parentAuxPath = Join-Path $repoRoot "main.aux"
    $parentLogPath = Join-Path $repoRoot "main.log"
    $parentBblPath = Join-Path $repoRoot "main.bbl"
    $parentBlgPath = Join-Path $repoRoot "main.blg"
